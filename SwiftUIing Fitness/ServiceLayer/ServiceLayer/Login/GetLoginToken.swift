...
open class GetLoginToken: RemoteUploader<LoginResponse?> {}

extension GetLoginToken {
    ...
    private static func map(_ data: Data, from response: HTTPURLResponse) throws -> LoginResponse? {
        if response.isOK {
            do {
                let baseResponse = try JSONDecoder().decode(LoginResponse.self, from: data)
                return baseResponse
